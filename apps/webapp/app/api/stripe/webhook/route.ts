    ...
    if (!user.data?.bot_welcome_credit_given) {
      try {
        await stripe.billing.creditGrants.create({
          ...
          amount: { type: 'monetary', value: { currency: 'usd', amount: INITIAL_BOT_CREDIT_CENTS } },
    ...
    if (!user.data?.[creditKey]) {
      try {
        await stripe.billing.creditGrants.create({
          ...
          category: 'promotional',
          amount: { type: 'monetary', value: { currency: 'usd', amount: TX_WELCOME_CREDIT_CENTS } },
          applicability_config: { scope: { price_type: 'metered' } },
        } as Stripe.Billing.CreditGrantCreateParams)
...
async function handleCheckoutCompleted(stripe: Stripe, session: Stripe.Checkout.Session) {
  ...
  try {
    await stripe.billing.creditGrants.create({
      ...
      category: 'paid',
      amount: { type: 'monetary', value: { currency: 'usd', amount: topupCents } },
      applicability_config: { scope: { price_type: 'metered' } },
    } as Stripe.Billing.CreditGrantCreateParams)
  ...
  try {
    ...
    if (pi.status === 'succeeded') {
      await stripe.billing.creditGrants.create({
        ...
        category: 'paid',
        amount: { type: 'monetary', value: { currency: 'usd', amount: topupAmountCents } },
        applicability_config: { scope: { price_type: 'metered' } },
      } as Stripe.Billing.CreditGrantCreateParams)
