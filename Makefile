...
vars: ## Update all variables: secrets from .env + validate terraform.tfvars + import existing resources
	...
	@if [ -f deployment/terraform/environments/$(ENV)/terraform.tfvars ]; then \
		...
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.billing.google_service_account.service_account projects/$(PROJECT_ID)/serviceAccounts/$(ENV)-billing@$(PROJECT_ID).iam.gserviceaccount.com 2>/dev/null && echo "  ✓ billing service account" || echo "  ⚠ billing service account" && \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.webapp.google_service_account.service_account projects/$(PROJECT_ID)/serviceAccounts/$(ENV)-webapp@$(PROJECT_ID).iam.gserviceaccount.com 2>/dev/null && echo "  ✓ webapp service account" || echo "  ⚠ webapp service account" && \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.billing.google_cloud_run_v2_service.service projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-billing 2>/dev/null && echo "  ✓ billing Cloud Run service" || echo "  ⚠ billing Cloud Run service" && \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.webapp.google_cloud_run_v2_service.service projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-webapp 2>/dev/null && echo "  ✓ webapp Cloud Run service" || echo "  ⚠ webapp Cloud Run service" && \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false 'module.billing.google_cloud_run_v2_service_iam_member.public_access[0]' "projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-billing roles/run.invoker allUsers" 2>/dev/null && echo "  ✓ billing IAM member" || echo "  ⚠ billing IAM member" && \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false 'module.webapp.google_cloud_run_v2_service_iam_member.public_access[0]' "projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-webapp roles/run.invoker allUsers" 2>/dev/null && echo "  ✓ webapp IAM member" || echo "  ⚠ webapp IAM member" && \
...
deploy: ## Deploy to Cloud Run
	...
	@cd deployment/terraform && ( \
		...
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.billing.google_service_account.service_account projects/$(PROJECT_ID)/serviceAccounts/$(ENV)-billing@$(PROJECT_ID).iam.gserviceaccount.com >/dev/null 2>&1 || true; \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.webapp.google_service_account.service_account projects/$(PROJECT_ID)/serviceAccounts/$(ENV)-webapp@$(PROJECT_ID).iam.gserviceaccount.com >/dev/null 2>&1 || true; \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.billing.google_cloud_run_v2_service.service projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-billing >/dev/null 2>&1 || true; \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false module.webapp.google_cloud_run_v2_service.service projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-webapp >/dev/null 2>&1 || true; \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false 'module.billing.google_cloud_run_v2_service_iam_member.public_access[0]' "projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-billing roles/run.invoker allUsers" >/dev/null 2>&1 || true; \
		terraform import -var-file="environments/$(ENV)/terraform.tfvars" -input=false 'module.webapp.google_cloud_run_v2_service_iam_member.public_access[0]' "projects/$(PROJECT_ID)/locations/$(REGION)/services/$(ENV)-webapp roles/run.invoker allUsers" >/dev/null 2>&1 || true; \
	)
